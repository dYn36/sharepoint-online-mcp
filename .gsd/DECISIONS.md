...
| 016 | 2026-03-16 | client | Testability of HTTP error paths in SharePointClient | Add optional `options.fetchFn` constructor parameter with default to module-imported `node-fetch` | Same injectable pattern as D010 for discoverTenantId. Enables testing 401/403/404 error messages without module mocking flags. Existing tests that catch-and-ignore fetch errors still work unchanged. | No |
| 017 | 2026-03-16 | auth | Error wrapping strategy | Export pure `wrapAuthError(error)` function, call it from device code flow catch block | Pure function is independently testable without mocking MSAL. Detects AADSTS codes via regex on both `errorCode` and `message` properties. | No |
| 018 | 2026-03-16 | dist | npm files whitelist strategy | Use `files: ["src/", "README.md", "LICENSE"]` with `.npmignore` as belt-and-suspenders | `src/` directory inclusion automatically picks up future source files. `.npmignore` prevents accidental leaks if `files` field is ever removed. | No |
